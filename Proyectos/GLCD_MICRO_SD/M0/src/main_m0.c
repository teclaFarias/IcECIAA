...
//		sprintf_mio(aux,"\n\r indiceFunciones: %d   valorFuncion: %d ", indice, valor);
//		DEBUGSTR(aux);	
//		limpiaBufferAUX();
		return true;
	}
...
/*
		bloqueTouch( &coordX , &coordY );

		if(bloque0){
			nuevoTanque(cifra0 , 0, 0, 239, 159,0,0,0,1,255,1);
			Led_OFF(2);
			bloque0= false;
			nuevoTanque(cifra1 , 0, 160, 239, 159,0,0,0,25,25,25);
		}
		...
		if(bloque1){
			...
			Led_ON(2);
			bloque1= false;
			nuevoTanque(cifra0 , 0, 0, 239, 159,0,0,0,25,25,25);
		}
*/
		interpretarComando();
