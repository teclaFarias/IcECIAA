...
void printBitmap(int x, int y, int ancho, int alto, unsigned long int addressImg){
	unsigned int col;
	int tx=0;
	int ty=0;
	int pixel=0;
	...
	for (ty=0; ty < (alto + 1); ty++){
		...
		for (tx=0; tx <= (ancho); tx++){
			ch1= MemoriaIPC.mensaje[elementoSD];
			ch2= MemoriaIPC.mensaje[elementoSD + 1];
			elementoSD += 2;
			...
			if(elementoSD == tamanoArray){  // cantidad elementos en el bloque de memoria
				contBloque++;
				elementoSD=0;
				//readBlockSD(addressImg + (contBloque* BUFFER_SERIE));  // actualiza array str[]
				//direccionIncrementada= ;
				cargarArraySD(addressImg + (20*contBloque* BUFFER_SERIE) );
			}				
