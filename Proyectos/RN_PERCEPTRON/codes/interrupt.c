...
//Interpretación de Comando:
	if((caracterRx_UART2 == COMANDO) && (contadorRx_UART2 > 0)){  //Caracter Fin de Cadena "$" ASCII: 36 
		tipo_dato_UART2= COMANDO;
		contadorRx_UART2=0;
	}
//Interpretación de Valor:
	if((caracterRx_UART2 == VALOR) && (contadorRx_UART2 > 0)){  //Caracter Fin de Cadena "#" ASCII: 35
		tipo_dato_UART2= VALOR;
		contadorRx_UART2=0;
	}
	else{
		*(pRx_UART2 + contadorRx_UART2) = caracterRx_UART2;  //Guardamos el caracter en el vector
		tipo_dato_UART2= caracterRx_UART2;
		caracterRx_UART2="";  //Limpio la variable
		contadorRx_UART2++;
		if(contadorRx_UART2 == BUFFER_RX_UART2){
			contadorRx_UART2=0;
			pRx_UART2= dataRx_UART2;
		}
	}
