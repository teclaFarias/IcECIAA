...
void chipSelectLow(void){
	chipSelectTouchHigh();  // Deshabilito el TouchScreen
...
/*
	...
	sprintf_mio(aux,"Sector size = %d bytes. # sectors = %d \n",(unsigned int)BYTES_PER_SECTOR, (unsigned int)NSEC);
	DEBUGSTR(aux);	
	limpiaBufferAUX();
	...
	if(comandoR1(17, address ,0xFF)){
      ...
      while(length < BUFFER_SERIE){
		...
		if(comandoR1(0x4C,0x00000000,0x00)){   //COMMAND12 MANDATORY
			return true;  // La transacción fue exitosa, termina la función
		}
      //delay_rit_ms(1000);
      length = 0;
      i = 0;
...
bool writeMultiBlockSD(unsigned long int address){
	int j=0;
	int k=0;
	...
	for(j=0; j < 20; j++){
		for(k=0; k < BUFFER_SERIE; k++){
			str[k]= MemDisplay.buffer[offset];
			offset++;
		}
		writeBlockSD(address, &str);
		address= address + BUFFER_SERIE;
		delay_rit_ms(100); //tiempo necesario para el copiado, en menos tiempo falla

	}
	sprintf_mio(aux,"OK|20 Bloques copiados\n");
	DEBUGSTR(aux);	
	limpiaBufferAUX();
	return true;
}
...
bool cargarArraySD(unsigned long int address){
	int i=0;
	unsigned int elemento=0;
	for(i=0; i < 20; i++){		
		if(!readBlockSD(address)){
			sprintf_mio(aux,"Ocurrio ERROR  mientras intentaba cargar ARRAY al leer el Bloque: %d \n", address);
			DEBUGSTR(aux);	
			limpiaBufferAUX();
			return false;
		}
		for(int b=0; b< BUFFER_SERIE; b++){
			elemento= (unsigned int) ((i * BUFFER_SERIE) + b); 
			MemDisplay.buffer[elemento]= str[b];
		}
		address = address + BUFFER_SERIE;
	} 
	return true;
...
bool cargarIconoSD(unsigned long int bloque){
	int i=0;
	for(i=0; i < 15; i++){		
		if(!readBlockSD(bloque)){
			sprintf_mio(aux,"\r\n Ocurrio ERROR mientras intentaba cargar ICONO al leer el Bloque: %d \r\n", i);
			DEBUGSTR(aux);	
			limpiaBufferAUX();
			return false;
		}
		for(int b=0; b< BUFFER_SERIE; b++)
			MemDisplay.buffer[(i* BUFFER_SERIE)+b]= str[b];
		bloque += BUFFER_SERIE;
	} 
	return true;
}
