...
bool comandoR1(unsigned char comando, unsigned int argumento, unsigned char crc){
	int i = 0, count = 0;
   unsigned char buff=0x00;
	unsigned char intentosConexion=5;
   do{
...
bool writeBlockSD(unsigned int address, unsigned char * data ){
...
bool readBlockSD(unsigned int address){
	unsigned char buff=0, readdata=0;
	int length=0,count=0,i=0;
   dummy_clocks(8);    
   count = 6;
	if(comandoR1(0x51, address ,0xFF)){
	   length = 0;
      while(length<512){
	      str[length]=response();  //read buffer (R1) should be 0x01 = idle mode   
         length++;
...
bool testWRSD(unsigned char accion){
	unsigned char datos[512];
	for(int i=0; i < 512; i++)
		datos[i]= 0x35;
	if(accion == 1){
		if(writeBlockSD(512*4, &datos)){
			sprintf_mio(aux,"\r\n Escritura de Bloque: OK \r\n");
			DEBUGSTR(aux);	
			limpiaBufferAUX();
		}
		else{
			sprintf_mio(aux,"\r\n Escritura de Bloque: Error \r\n");
			DEBUGSTR(aux);	
			limpiaBufferAUX();
		}
		for(int i=0; i < 512; i++)
		...
		if(readBlockSD(512*3)){
			sprintf_mio(aux,"\r\n Lectura de Bloque: OK \r\n");
			DEBUGSTR(aux);	
			limpiaBufferAUX();
		}
		else{
			sprintf_mio(aux,"\r\n Lectura de Bloque: Error \r\n");
			DEBUGSTR(aux);	
			limpiaBufferAUX();
		}
		dump_bytes(str, 512);
		if(readBlockSD(512*4)){
			sprintf_mio(aux,"\r\n Lectura de Bloque: OK \r\n");
			DEBUGSTR(aux);	
			limpiaBufferAUX();
		}
